...
(function (root, factory) {
  if (typeof module !== 'undefined' && module.exports) {
    ...
    // Browser global
    root.ACMIClient = factory(root);
  }
}(typeof self !== 'undefined' ? self : typeof global !== 'undefined' ? global : Function('return this')(), function (root) {
  'use strict';
  ...
  ACMIClient.prototype._request = function (tool, params, opts) {
    ...
    function doFetch() {
      return new Promise(function (resolve, reject) {
        ...
        if (!fetchFn) {
          clearTimeout(timer);
          reject(new Error('fetch is not available — provide a polyfill or use a browser/Node 18+ environment'));
          return;
        }
        ...
        fetchFn(self._config.endpoint, fetchOpts).then(function (response) {
          clearTimeout(timer);
          if (!response.ok) {
  ...
  ACMIClient.prototype.cat = function (keys, opts) {
    ...
    var cacheKey = 'cat:' + keys.slice().sort().join(',') + ':' + (opts.since || 'all') + ':' + (opts.limit || 50);
    if (opts.force) {
   ...
   * Loads: all agents, work items, tasks, notes, events, docs,
   ...
   * @param {string} [wsUrl] - WebSocket URL (auto-detected if omitted)
   */
  ACMIClient.prototype.connectWebSocket = function (wsUrl) {
    ...
    if (!wsUrl) {
      ...
      var host = typeof window !== 'undefined' && window.location ? (window.location.host || 'localhost:8080') : 'localhost:8080';
